...
(function () {
  ...
  function updateBadge() {
    var unread = pushLog.filter(function (p) { return readPushes.indexOf(p.id) < 0; }).length;
  ...
  function enqueuePush(item) {
    if (pushSeen.has(item.id)) return;
    pushSeen.add(item.id);
    pushLog.unshift({ id: uid(), ts: Date.now(), title: item.title, type: item.type, newsId: item.id, status: 'pending' });
    if (pushLog.length > 50) pushLog.length = 50;
  ...
  function renderPush() {
    $('pushList').innerHTML = pushLog.map(function (p) {
      var st = p.status === 'sent' ? '✅ 已发送(企微)' : (p.status === 'fail' ? '⚠️ ' + esc(p.note || '发送失败') : (p.status === 'nosend' ? '未配置Webhook·本地记录' : '待发送'));
      return '<li class="push-item' + (p.status === 'nosend' ? ' dedupe' : '') + '">' +
        '<div class="pi-head"><span>💬 企业微信 · 重要消息</span><span>' + fmtClock(p.ts) + '</span><span class="pi-status" style="margin-left:auto">' + st + '</span></div>' +
        '<div class="pi-body"><div class="pi-title" data-act="card" data-id="' + esc(p.newsId) + '" style="cursor:pointer">' + esc(p.title) + '</div>' +
        '<div class="pi-line">' + esc(p.type) + ' · 点击打开详情 · 同事件合并推送 / 5分钟频控</div></div>' +
        '<div class="pi-foot"><span>推送对象：企微群机器人(WECOM_WEBHOOK)</span><button class="btn btn-outline btn-sm" data-act="push-send" data-push="' + p.id + '">发送</button></div></li>';
    }).join('') || '<li style="color:var(--muted);font-size:12.5px;padding:8px">暂无。出现符合规则的「重要消息」（停复牌/立案/业绩预告/重组/政策等）将自动入队。</li>';
  }
  async function sendPush(p) {
    var target = pushLog.find(function (x) { return x.id === p; });
    if (!target) return;
    target.status = 'sending'; renderPush();
    try {
      var r = await api('/api/push-send');
      if (MODE !== 'live') { target.status = 'nosend'; target.note = '演示模式'; }
      else if (r.sent) target.status = 'sent';
      else { target.status = 'nosend'; target.note = r.reason; }
    } catch (e) { target.status = 'nosend'; target.note = '未连接后端'; }
    renderPush();
  }
  function markAllRead() {
    pushLog.forEach(function (p) { if (readPushes.indexOf(p.id) < 0) readPushes.push(p.id); });
    saveLS('fn_readPush', readPushes); updateBadge();
  }
  ...
  /* ---------- 数据轮询（live） ---------- */
  var firstPollDone = false, initPushed = 0;
  async function pollFeed() {
    try {
      ...
      if (!firstPollDone) {           // 首次装载：仅把最新 4 条重要消息入推送队列，不弹提示
        firstPollDone = true;
        added.forEach(function (it) {
          if (it.imp && it.type !== '传闻' && initPushed < 4) { enqueuePush(it); initPushed++; }
        });
        return true;
      }
      added.forEach(function (it) {   // 后续轮询：新到达 → 提示 + 重要入队
        toast('⚡ 新资讯：' + (it.title || '').slice(0, 30) + '…', 'info');
        if (it.imp && it.type !== '传闻') enqueuePush(it);
      });
      return true;
  ...
  function bindDelegation() {
    document.addEventListener('click', function (e) {
      ...
      else if (act === 'push-send') { sendPush(t.dataset.push); }
